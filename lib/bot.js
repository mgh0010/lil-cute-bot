...
function getHighAndMin() {
    ...
    request(url, function (err, response, body) {
      if(err){
        ...
        tempRange['high'] = list[0].temp.max;
        tempRange['low'] = list[0].temp.min;
        tempRange['day'] = list[0].temp.day;
      }
      weatherInfo += `High of ${tempRange['high']} 
                      Low of ${tempRange['low']}`;
