    ...
    if(err) {
      ...
      request(requestUrl, function(err, response, body) {
        if(err) {
          ...
          let dailyMax = bodyObj.daily.data[0].temperatureHigh;
          let dailyMin = bodyObj.daily.data[0].temperatureLow;
          weatherInfo = `Address: ${addressLatLongDict['address']}\n`;
          weatherInfo += 'Currently: ' + bodyObj.currently.temperature + '\n';
          weatherInfo += 'Low: ' + dailyMin + '\n';
          weatherInfo += 'High: ' + dailyMax + '\n';
...
function checkMessage(message) {
  ...
  if (messageText && shrugRegex.test(messageText)) {return '¯\\_(ツ)_/¯';}
    ...
    getBibleESVResponse(messageText);
      ...
      else if (messageText.includes("new weather aub"))
      {
        getDarkSkyWeather('auburn, al');
      }
      else {
        let location = messageText.replace(/new weather /, '');
        location = location.replace(/\s/g, ',+');
        getDarkSkyWeather(location);
      }
      return null;
  }
