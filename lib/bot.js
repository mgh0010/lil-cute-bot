    ...
    if(err) {
      ...
      request(requestUrl, function(err, response, body) {
        if(err) {
          ...
          let dailyMax = bodyObj.daily[0].temperatureHigh;
          let dailyMin = bodyObj.daily[0].temperatureLow;
  ...
  if (messageText && shrugRegex.test(messageText)) {return '¯\\_(ツ)_/¯';}
    ...
    getBibleESVResponse(messageText);
      ...
      else if (messageText.includes("new weather aub"))
      {
        ...
        messageText = messageText.replace(/new weather /, '');
        messageText = messageText.replace(/\s/g, ',+');
        getDarkSkyWeather(messageText);
      }
      return null;
  }
