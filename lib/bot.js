...
function getPiedmontWeather() {
    weatherInfo = 'If you see this, it is not working';
    let url = `http://api.openweathermap.org/data/2.5/weather?id=4083004&units=imperial&appid=${apiKey}`
    let urlminMax = `http://api.openweathermap.org/data/2.5/forecast/daily?id=4083004&units=imperial&appid=${apiKey}`
    request(url, function (err, response, body) {
      if(err){
        weatherInfo = err;
      } 
...
function getHooverWeather() {
    weatherInfo = 'If you see this, it is not working';
    let url = `http://api.openweathermap.org/data/2.5/weather?id=4067994&units=imperial&appid=${apiKey}`
    let urlminMax = `http://api.openweathermap.org/data/2.5/forecast/daily?id=4067994&units=imperial&appid=${apiKey}`
    request(url, function (err, response, body) {
      if(err){
        weatherInfo = err;
      } 
...
function getHighAndMin(dayAsString, daysFromNow) {
...
function getForecastHighLows(daysToLookAhead=3) {
  let d = new Date();
...
function showWhatTheyCanDo() {
  ...
  for (regexPattern in regexPatterns) {
    whatTheyCanDo += `"${regexPattern.toString()}"\n`;
    //regex pattern to string
  }
