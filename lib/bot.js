...
function getWeather() {
    ...
    request(url, function (err, response, body) {
      if(err){
        ...
        weatherInfo = `${weather.weather[0].description} with a temp of ${weather.main.temp} degrees in ${weather.name}!`;
...
function getTodaysWeather() {
    ...
    request(url, function (err, response, body) {
      if(err){
        ...
        weatherInfo = '';
        list.forEach((threeHourWeather) --> {
            weatherInfo += threeHourWeather.weather[0].description;
            weatherInfo += '\n'
        })
        weatherInfo = weather.cod;
        sendMessage(weatherInfo);
      }
    });
}
